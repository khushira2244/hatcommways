...
def test_join_advisory_partial_approval_notification_tree_and_my_events(database,service):
    ...
    discovered=client.get('/events/discover',headers=ah);assert discovered.status_code==200
    result=next(item for item in discovered.json() if item['id']==str(event.id))
    assert result['name']==event.name and result['location_description']==event.location_description
