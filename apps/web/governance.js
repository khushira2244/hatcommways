...
function render() {
  ...
  host.innerHTML = govState.items.map(item => `<article class="gov-item"><header><h2>${esc(item.label)}</h2><span>${esc(labels[item.status] || item.status)}</span></header>${item.reason ? `<p><b>Why this was suggested:</b> ${esc(item.reason)}</p>` : ''}${item.suggested_documents.length ? `<small>Suggested information or evidence</small><ul>${item.suggested_documents.map(document => `<li>${esc(document)}</li>`).join('')}</ul>` : ''}<small>${item.knowledge_type === 'VERIFIED_REQUIREMENT' ? 'Verified requirement source provided' : 'Guidance only; not presented as a verified legal requirement'}</small>${item.evidence.length ? `<section class="submitted-evidence"><b>Evidence submitted</b><ul>${item.evidence.map(evidenceMarkup).join('')}</ul></section>` : ''}<form class="item-form" data-item="${item.id}"><label>Organizer note<textarea name="organizer_note">${esc(item.organizer_note || '')}</textarea></label><div class="item-actions"><button name="save">Add information</button><button class="secondary" name="evidence" type="button">Add PDF / Image Proof</button><button class="secondary" name="na" type="button">Not applicable</button></div></form><form class="evidence-form" data-evidence-for="${item.id}" hidden><label>Evidence type<select name="evidence_type"><option value="FILE">Upload PDF or image</option><option value="URL">URL</option><option value="REFERENCE_NUMBER">Reference number</option><option value="TEXT_CONFIRMATION">Text confirmation</option></select></label><label class="evidence-file">Choose proof file <small>PDF, JPG, PNG or WebP · maximum 10 MiB</small><input name="file" type="file" accept=".pdf,.jpg,.jpeg,.png,.webp,application/pdf,image/jpeg,image/png,image/webp"></label><label class="evidence-value" hidden>Evidence value<input name="value_or_reference"></label><label>Evidence label (optional)<input name="label" maxlength="200"></label><label>Note (optional)<textarea name="note" maxlength="2000"></textarea></label><div class="item-actions"><button>Upload Proof</button><button class="secondary" name="cancel" type="button">Cancel</button></div></form></article>`).join('');
  host.querySelectorAll('.item-form').forEach(wireItem);
  host.querySelectorAll('.evidence-form').forEach(wireEvidence);
  const submissionLocked = ['UNDER_REVIEW','SUBMITTED'].includes(assessment.organizer_visible_status);
  document.querySelector('#submit-governance').disabled = submissionLocked;
  document.querySelector('#show-add').disabled = submissionLocked;
  document.querySelectorAll('#add-item input, #add-item select, #add-item textarea, #add-item button, #gov-items input, #gov-items select, #gov-items textarea, #gov-items button').forEach(control => {
    control.disabled = submissionLocked;
  });
  if (submissionLocked) {
    document.querySelector('#add-item').hidden = true;
    document.querySelectorAll('.evidence-form').forEach(form => { form.hidden = true; });
    document.querySelector('#gov-message').textContent = 'Governance information submitted. Continue to Stage Planning when ready.';
  }
}
...
document.querySelector('#submit-governance').onclick = async () => {
  const button = document.querySelector('#submit-governance');
  button.disabled = true;
  try {
    govState = await api(`/events/${govEventId}/governance/submit`, {method:'POST',body:JSON.stringify({expected_version:govState.assessment.version,idempotency_key:crypto.randomUUID()})});
    render();
  } catch (error) {
    document.querySelector('#gov-message').textContent = error.message;
