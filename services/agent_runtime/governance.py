...
class StrandsGovernanceAgent:
    ...
    def generate(self,event_id:UUID,organizer_id:UUID)->GovernanceProposal:
        ...
        model=BedrockModel(boto_session=boto3.Session(profile_name=os.environ.get('AWS_PROFILE'),region_name=REGION),boto_client_config=Config(connect_timeout=10,read_timeout=120,retries={'max_attempts':2,'mode':'standard'}),model_id=MODEL_ID,temperature=0,max_tokens=1800)
